___INFO___

{
  "displayName": "Flashtalking Activity Tracker",
  "description": "Flashtalking capturing website activity.",
  "securityGroups": [],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/png;base64,",
    "displayName": "",
    "id": "brand_dummy"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "help": "please enter the same name of the spot as encoded in flashtalking",
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ],
    "displayName": "Spot Name",
    "defaultValue": "web_analytics",
    "simpleValueType": true,
    "name": "spotName",
    "type": "TEXT",
    "valueHint": "e.g. web_analytics"
  },
  {
    "help": "You can find the container ID by downloading the code from flashtalking and look for the range of number in the 'src' section (i.e. 123456;321654;789456')",
    "alwaysInSummary": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      },
      {
        "args": [
          "(.*)\\/imp\\/[0-9]\\/[0-9;]{5,}(.*)"
        ],
        "errorMessage": "the url you enter is not valid, please copy/paste the impression tracker from flashtalking.",
        "type": "REGEX"
      }
    ],
    "displayName": "Flashtalking Impression Tracker",
    "simpleValueType": true,
    "name": "impressionTracker",
    "type": "TEXT",
    "valueHint": "https://servedby.flashtalking.com/imp/2/105340;3661411;201;pixel;MediascaleBE;cookieMatchimp/?cachebuster=[CACHEBUSTER]"
  },
  {
    "displayName": "Cachebuster",
    "simpleValueType": true,
    "name": "cacheBuster",
    "type": "TEXT"
  },
  {
    "displayName": "Essential Variables",
    "name": "ftEssentials",
    "groupStyle": "ZIPPY_OPEN",
    "type": "GROUP",
    "subParams": [
      {
        "valueValidators": [],
        "displayName": "Essentials Parameters",
        "name": "ftEparameters",
        "simpleTableColumns": [
          {
            "selectItems": [
              {
                "displayValue": "Consent String",
                "value": "u1"
              },
              {
                "displayValue": "Debug Toggle",
                "value": "u2"
              },
              {
                "displayValue": "Event Name",
                "value": "u3"
              },
              {
                "displayValue": "Language",
                "value": "u4"
              },
              {
                "displayValue": "Page URL (URI encoded)",
                "value": "u5"
              },
              {
                "displayValue": "Referrer",
                "value": "u6"
              },
              {
                "displayValue": "User Identifiers (json)",
                "value": "u7"
              },
              {
                "displayValue": "u8",
                "value": "u8"
              },
              {
                "displayValue": "u9",
                "value": "u9"
              },
              {
                "displayValue": "Timestamp (epoc)",
                "value": "u10"
              }
            ],
            "defaultValue": "",
            "displayName": "Key",
            "name": "key",
            "isUnique": true,
            "type": "SELECT"
          },
          {
            "defaultValue": "",
            "displayName": "value",
            "name": "value",
            "type": "TEXT"
          }
        ],
        "type": "SIMPLE_TABLE"
      }
    ]
  },
  {
    "displayName": "Flashtalking Transaction Values",
    "name": "ftXtransaction",
    "groupStyle": "ZIPPY_CLOSED",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "",
        "name": "ftXparameters",
        "simpleTableColumns": [
          {
            "selectItems": [
              {
                "displayValue": "Transaction Reference ID",
                "value": "ftXRef"
              },
              {
                "displayValue": "Transaction Name",
                "value": "ftXName"
              },
              {
                "displayValue": "Transaction Quantity",
                "value": "ftXNumItems"
              },
              {
                "displayValue": "Transaction Currency",
                "value": "ftXCurrency"
              },
              {
                "displayValue": "Transaction Type",
                "value": "ftXType"
              },
              {
                "displayValue": "Transaction Value",
                "value": "ftXValue"
              }
            ],
            "defaultValue": "",
            "displayName": "key",
            "name": "key",
            "isUnique": true,
            "type": "SELECT"
          },
          {
            "defaultValue": "",
            "displayName": "value",
            "name": "value",
            "type": "TEXT"
          }
        ],
        "type": "SIMPLE_TABLE",
        "newRowButtonText": "New Transaction Feature"
      }
    ]
  },
  {
    "displayName": "Flashtalking Custom Parameters",
    "name": "ftCustom",
    "groupStyle": "ZIPPY_CLOSED",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "",
        "name": "ftCparameters",
        "simpleTableColumns": [
          {
            "selectItems": [
              {
                "displayValue": "u11",
                "value": "u11"
              },
              {
                "displayValue": "u12",
                "value": "u12"
              },
              {
                "displayValue": "u13",
                "value": "u13"
              },
              {
                "displayValue": "u14",
                "value": "u14"
              },
              {
                "displayValue": "u15",
                "value": "u15"
              },
              {
                "displayValue": "u16",
                "value": "u16"
              },
              {
                "displayValue": "u17",
                "value": "u17"
              },
              {
                "displayValue": "u18",
                "value": "u18"
              },
              {
                "displayValue": "u19",
                "value": "u19"
              },
              {
                "displayValue": "u20",
                "value": "u20"
              }
            ],
            "defaultValue": "",
            "displayName": "Parameter Index",
            "name": "key",
            "isUnique": true,
            "type": "SELECT"
          },
          {
            "defaultValue": "",
            "displayName": "Value",
            "name": "value",
            "type": "TEXT"
          }
        ],
        "type": "SIMPLE_TABLE",
        "newRowButtonText": "New Parameter"
      }
    ]
  }
]


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "send_pixel",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://servedby.flashtalking.com/imp/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// Enter your template code here.
const log = require('logToConsole');
const sendPixel = require('sendPixel');
const encodeUriComponent  = require('encodeUriComponent');
const random = data.cacheBuster;
log('data =', data);

// Defining Variables
var sourceURL = data.impressionTracker;
	sourceURL = sourceURL.replace('[CACHEBUSTER]',random);

/* Do not need right now. 
var advertiserId = containerId.split(';')[0];
var spotGroupId = containerId.split(';')[1];
var spotId = containerId.split(';')[2];
*/

var trackingUrl = sourceURL + '&ft_custom=' + encodeUriComponent('{') ;
var ftCustom = '';

if(data.ftEparameters){
  for (var i = 0; i <= data.ftEparameters.length-1; ++i){
      ftCustom += '"' + data.ftEparameters[i].key + '"' + ':' + '"' + data.ftEparameters[i].value + '"' + ','; 
  }
}

if(data.ftXparameters){
  for (var i = 0; i <= data.ftXparameters.length-1; ++i){
      ftCustom += '"' + data.ftXparameters[i].key + '"' + ':' + '"' + data.ftXparameters[i].value + '"' + ','; 
  }
}

if(data.ftCparameters){
  for (var i = 0; i <= data.ftCparameters.length-1; ++i){
      ftCustom += '"' + data.ftCparameters[i].key + '"' +  ':' + '"' + data.ftCparameters[i].key + '"' + ','; 
  }
}

ftCustom = ftCustom.slice(0,-1);
ftCustom += '}';

ftCustom = encodeUriComponent(ftCustom);
trackingUrl += ftCustom; 

log(trackingUrl);

sendPixel(trackingUrl,data.gtmOnSuccess,data.gtmOnFailure);


___NOTES___

Created on 31/05/2019, 14:45:21
